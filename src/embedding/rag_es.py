...
from src.tools import get_abrreviate, get_title_private, get_file_img
...
class RAG:
    ...
    async def apreprocess_query(self, query: str) -> str:
        ...
        prompt = ChatPromptTemplate(
            message_templates=[
                ...
                ChatMessage(
                    role="user",
                    content=validate_query_prompt.format(
                        abbreviations_str=ABBREVIATION_LIST,
                        query_str=query,
                    ),
                ),
        ...
        llm_o1 = OpenAI(
            model="gpt-4o-mini", temperature=0.1, top_p=0.2, max_tokens=1000
        )

        expanded_query = (await llm_o1.achat(messages)).message.content
    ...
    def contextual_rag_search(
        ...
        if "điểm chuẩn" in query:
            query = query.replace("điểm chuẩn", "điểm tuyển sinh")
        ...
        retrieved_nodes = sorted(combined_nodes, key=lambda x: x.score, reverse=True)
    ...
    async def acontextual_rag_search(
        self, query: str, k: int = 5, debug: bool = False, use_compressor: bool = False
    ) -> str:
        """
        Search the query with the Contextual RAG.
        ...
        if "điểm chuẩn" in query:
            if "tuyển sinh" not in query:
                query = query.replace("điểm chuẩn", "điểm chuẩn tuyển sinh")
            if "ngành" not in query:
                query += " các ngành"
        
        if "năm nay" in query:
            year = datetime.now().year
            query = query.replace("năm nay", f"năm {year}")

        if "chỉ tiêu" in query and "chỉ tiêu tuyển sinh" not in query:
            query = query.replace("chỉ tiêu", "chỉ tiêu tuyển sinh")
            if "ngành" not in query:
                query += " các ngành"

        start_time = time.time()
        dict_filename, content_nodes = {}, {}
        semantic_doc_id, bm25_doc_id = [], []
        node_ids_es = set()
        ...
        for result in bm25_results:
                ...
                scroll_filter=models.Filter(
                    must=[
                        models.FieldCondition(
                            key="file_name", match=models.MatchValue(value=result.file_name)
                        )
                    ]
                ),
                limit=50,
            ...
            for point in response[0]:
                str_to_dict = point.payload["_node_content"]
                data = json.loads(str_to_dict)
                doc_id = data.get("metadata", {}).get("doc_id")
                if doc_id == result.doc_id:
                    node_ids_es.add(point.id)
                
            node_ids = list(node_ids_es)
            contents = vector_store.get_nodes(node_ids=node_ids)
            content_node = ""
            for node in natsorted(contents, lambda x: x.start_char_idx):
                content_node += node.text + " "

            content_nodes[result.doc_id] = content_node
        ...
        start_time = time.time()
        # semantic_results: Response = query_engine.query(query)
        semantic_results: Response = retriever.retrieve(query)

        nodes = semantic_results

        for node in nodes:
            # doc_id is key of payload
            doc_id = node.node.ref_doc_id
            
            if content_nodes.get(node.metadata["doc_id"]) is not None:
                continue
            ...
            node_ids = [point.id for point in response[0]]
            contents = vector_store.get_nodes(node_ids=node_ids)
            content_node = ""
            for node in natsorted(contents, lambda x: x.start_char_idx):
                content_node += node.text + " "

            content_nodes[node.metadata["doc_id"]] = content_node

        logger.info(f"Done Semantic Querying ..., search semantic {time.time() - start_time}")
        ...
        for n in retrieved_nodes:
            ...
            document_retrieved = document_retrieved.replace("\t", " ")
            document_retrieved = document_retrieved.replace("\xa0", " ")
            document_retrieved = re.sub(' +', ' ', document_retrieved)
        ...
        if "mã trường" in query or "Mã trường" in query:
            context_str = "Chunk: Mã trường UIT là QSC \n Resource: https://cdn.thuvienphapluat.vn/uploads/Hoidapphapluat/2024/NTKL/21112024/de-an-tuyen-sinh-truong-uit.pdf ======================== \n\n"
        ...
        logger.info(f"Một vài từ viết tắt quan trọng của UIT: {ABBREVIATION_LIST} \n\n =============== \n" + context_str)
        
        return (
            f"Một vài từ viết tắt quan trọng của UIT: {ABBREVIATION_LIST} \n\n =============== \n"
            + context_str
        )
