...
from rag_config import rag, rag_2025, guardrail_agent
...
# @input_guardrail
# async def system_guardrail(
#     ctx: RunContextWrapper[None], agent: Agent, input: str | list[TResponseInputItem]
# ) -> GuardrailFunctionOutput:
#     """
#     Guardrail function to ensure the input is related to university admissions.
#     If the input is related to university admissions, it will return the final output.
#     If the input is not related to university admissions, it will return a message indicating that the input is not related to university admissions.
#     """
#     result = await Runner.run(guardrail_agent, input, context=ctx.context)

#     print(f"Result from guardrail: {result.final_output}")

#     return GuardrailFunctionOutput(
#         output_info=result.final_output,
#         tripwire_triggered=result.final_output.is_related_to_admission,
#     )
...
async def run(message: cl.Message):
    try:
        ...
        agent = Agent(
            ...
            model="gpt-4.1-mini",
            # input_guardrails=[system_guardrail],
        )
